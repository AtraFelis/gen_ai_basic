<script lang="ts">
	...
	async function sendPrompt() {
		...
		try {
			...
			//2. 전달 :: 생성된 이미지 URL 전달 받기
			const { url: image_url } = await res.json();

			//3. 출력 :: 이미지 URL을 div에 출력
			url = image_url;
			await tick(); // DOM 업데이트를 기다립니다.
		} catch (e) {
		...
		<div class="side_wrap">
			<div class="title_wrap">
				...
				<div class="select_wrap">
					<select id="model" class="select_model" bind:value={selected}>
...
<style>
	...
	.common_panel {
		...
		box-shadow:
			0 0.4rem 1rem 0 rgba(0, 0, 0, 0.2),
			0 0.4rem 2rem 0 rgba(0, 0, 0, 0.19);
